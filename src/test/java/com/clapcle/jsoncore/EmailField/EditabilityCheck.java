...
class EditabilityCheck {
    ...
    void checkEditability_metWithCondition() throws IOException, URISyntaxException {
        ...
        String jsonContent = Files.readString(Paths.get(ClassLoader.getSystemResource("EmailField/SampleJson/editability_sample.json").toURI()));
        Form form = parseForm(jsonContent);

        String dataContent = Files.readString(Paths.get(ClassLoader.getSystemResource("EmailField/dataJson/editability_data.json").toURI()));
    ...
    void checkEditability_metWithConditionSuccess() throws IOException, URISyntaxException {
        ...
        String dataContent = Files.readString(Paths.get(ClassLoader.getSystemResource("EmailField/dataJson/edibility_data_success.json").toURI()));
        ObjectMapper objectMapper = new ObjectMapper();
        Map<String, Map<String, Object>> dataMap = objectMapper.readValue(dataContent, new TypeReference<Map<String, Map<String, Object>>>() {
        });
