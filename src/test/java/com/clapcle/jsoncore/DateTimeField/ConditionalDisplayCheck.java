...
import static org.junit.jupiter.api.Assertions.assertEquals;
 ...
 class ConditionalDisplayCheck {
    ...
    void checkConditionalDisplay_metWithCondition() throws IOException, URISyntaxException {
        ...
        String jsonContent = Files.readString(Paths.get(ClassLoader.getSystemResource("DateTimeField/SampleJson/conditionalDisplay_sample.json").toURI()));
        Form form = parseForm(jsonContent);

        String dataContent = Files.readString(Paths.get(ClassLoader.getSystemResource("DateTimeField/dataJson/conditionalDisplay_data.json").toURI()));
    ...
    void checkConditionalDisplay_metWithConditionSuccess() throws IOException, URISyntaxException {
        ...
        String dataContent = Files.readString(Paths.get(ClassLoader.getSystemResource("DateTimeField/dataJson/conditionalDisplay_data_success.json").toURI()));
        ObjectMapper objectMapper = new ObjectMapper();
        Map<String, Map<String, Object>> dataMap = objectMapper.readValue(dataContent, new TypeReference<Map<String, Map<String, Object>>>() {
        });
